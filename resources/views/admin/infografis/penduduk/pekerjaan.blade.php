...
@section('title', 'Data Kategori pekerjaan')

@section('content')
<main class="h-full pb-16 overflow-y-auto" x-data="kategoriPekerjaanHandler()">
  <div class="container grid px-6 mx-auto">
    ...
    @foreach($kategori->where('nm_kategori', 'pekerjaan') as $kat)
    <div class="mb-10">
  ...
  >
    <form action="/kategori-penduduk" method="POST" class="bg-white p-6 rounded-lg shadow-md w-96">
      @csrf
      <h3 class="text-lg font-semibold mb-3">Tambah Data Pekerjaan</h3>
  ...
  >
    <form x-bind:action="'/kategori-penduduk/' + dataItem.id_data" method="POST" class="bg-white p-6 rounded-lg shadow-md w-96">
      @csrf
      @method('PUT')
      <h3 class="text-lg font-semibold mb-3">Edit Data Pekerjaan</h3>
      <label>Nama Pekerjaan</label>
...
<script>
function kategoriPekerjaanHandler() {
  return {
    // state modal
    showAdd: false,
    showEdit: false,
