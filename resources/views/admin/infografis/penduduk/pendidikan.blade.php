...
@section('title', 'Data Kategori pendidikan')

@section('content')
<main class="h-full pb-16 overflow-y-auto" x-data="kategoriPendidikanHandler()">
  <div class="container grid px-6 mx-auto">
    ...
    @foreach($kategori->where('nm_kategori', 'pendidikan') as $kat)
    <div class="mb-10">
  ...
  >
    <form action="/kategori-penduduk" method="POST" class="bg-white p-6 rounded-lg shadow-md w-96">
      @csrf
      <h3 class="text-lg font-semibold mb-3">Tambah Data Pendidikan</h3>
  ...
  x-transition:enter="transition ease-out duration-150"
    ...
    <form x-bind:action="'/kategori-penduduk/' + dataItem.id_data" method="POST" class="bg-white p-6 rounded-lg shadow-md w-96">
      @csrf
      @method('PUT')
      <h3 class="text-lg font-semibold mb-3">Edit Data pendidikan</h3>
      <label>Nama pendidikan</label>
...
<script>
function kategoriPendidikanHandler() {
  return {
    // state modal
    showAdd: false,
    showEdit: false,
